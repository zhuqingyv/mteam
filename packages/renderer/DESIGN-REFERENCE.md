...
本文件按设计稿**画面**描述视觉，但 `CHANGELOG.md` 里的"设计决策记录 D-1 ~ D-13"**有覆盖权**。对前端落地影响较大的 2 条：

- **D-4（Logo 在线/离线灰度）**：设计稿 31/33/35/38/39/41 里 M Logo 左下都画了**绿色在线点**；但 D-4 决定**不用绿点角标**，改为"Logo 在线=彩色、离线=灰度"承载状态。执行时以 D-4 为准，绿点 StatusDot **不实现**在 Logo 上。
- **D-5（通知由 agent 签收）**：通知 UI **不提供用户 dismiss/忽略按钮**，签收由 agent 触发；前端只显示与点开跳转。

下面文本里"绿色在线点"等描述保留为"设计稿观察"，不代表落地要求；以每节的"落地提醒"为准。
...
**视觉构成**（从左到右）：
1. 品牌 M Logo（设计稿含绿色在线点；**落地按 D-4 改为 Logo 彩色=在线、灰度=离线，不加 StatusDot**），约 40px
2. TitleBlock：
...
```
┌─────────────────────────────────────────┐
│ 顶栏：Logo + M-TEAM (+D-4 灰度态)          ×│  ← ChatHeader
├─────────────────────────────────────────┤
│                                         │
│  [M] Claude                             │   ← 左气泡：agent
...
**视觉**：
- 小型浅玻璃胶囊（比主消息气泡更小、更浅）
- 左侧小 M Logo（~24px）（设计稿画了绿点，按 D-4 改为 Logo 彩色/灰度状态）
- 气泡内：
...
| 7 | 聊天窗顶栏 Logo 在线态用彩色、离线态灰度（D-4 覆盖设计稿的 StatusDot 绿点） | 40 | organism + atom `Logo` |
