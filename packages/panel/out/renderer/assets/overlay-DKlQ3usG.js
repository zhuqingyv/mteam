...
window.overlayBridge.onMessageEvents((msgs) => {
  const localNow = performance.now() / 1e3;
  messages = msgs.map((m) => ({
    from: m.from,
    to: m.to,
...
function findEdgeExit(cx, cy, nx, ny, bx, by, bw, bh, cornerR, maxDist, dpr) {
  const step = 2 * dpr;
...
function draw(ts) {
  ...
  for (const msg of messages) {
    ...
    let headPos, tailPos, fuseSrc, fuseDst;
    if (mt < 0.05) {
      headPos = mt / 0.05 * 0.1;
      tailPos = 0;
    } else if (mt < 0.4) {
      ...
      headPos = 1 + (mt - 0.8) / 0.2 * 0.05;
      tailPos = 1 + (mt - 0.8) / 0.2 * 0.05;
    }
    if (mt < 0.4) fuseSrc = 1;
    else if (mt < 0.6) fuseSrc = 1 - (mt - 0.4) / 0.2;
    else fuseSrc = 0;
    if (mt < 0.35) fuseDst = 0;
    else if (mt < 0.55) fuseDst = (mt - 0.35) / 0.2;
    else fuseDst = 1;
    const reach = Math.pow(Math.sin(mt * Math.PI), 0.65);
    lives.push({ fromIdx: fi, toIdx: ti, mt, headPos, tailPos, fuseSrc, fuseDst, reach });
  }
  const tentacles = [];
  const tentacleBoxPairs = [];
  for (const life of lives) {
    ...
    const o1 = Math.sin(time * 1.7 + life.fromIdx * 2.3 + life.toIdx * 1.1) * wobbleAmt + Math.sin(time * 2.9 + life.toIdx * 3.1) * wobbleAmt * 0.4;
    const o2 = Math.sin(time * 2.1 + life.toIdx * 1.7 + life.fromIdx * 2.9) * wobbleAmt + Math.cos(time * 1.3 + life.fromIdx * 2.7) * wobbleAmt * 0.4;
    tentacles.push({
      p0x,
      p0y,
      ...
      p3y,
      reach: life.reach,
      headPos: life.headPos,
      tailPos: life.tailPos,
      fuseSrc: life.fuseSrc,
      fuseDst: life.fuseDst,
      colorA: src.color,
      colorB: dst.color
    });
  ...
  for (let ti = 0; ti < tentacles.length; ti++) {
    ...
    gl.uniform4f(uTent[base + 7], 0, 0, 0, 0);
  }
